...
2. Rename dbconn-example.php to dbconn.php.
3. Put your database credentials into dbconn.php.
4. Make sure you have APC enabled.
5. Make the log file:

> $ cd /path/to/api/log && touch multi-attrs.php.log

6. Make sure the log can be written to. For example:

> $ cd /path/to/api/log && chmod 777 multi-attrs.php.log

7. Make sure the log won't grow forever. For example, use logrotate or make a cron job like:
